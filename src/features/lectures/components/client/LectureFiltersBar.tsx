"use client";
// src/features/lectures/components/client/LectureFiltersBar.tsx
...
interface LectureFiltersBarProps {
  ...
  currentFilters: LectureFilters;
}

const SORT_OPTIONS: { value: LectureSortOption; label: string }[] = [
...
export function LectureFiltersBar({
  ...
  scholars,
  currentFilters,
}: LectureFiltersBarProps) {
  const router = useRouter();
  const pathname = usePathname();
  const [isPending, startTransition] = useTransition();

  // Build a new URL from the current filters + an override
  const buildUrl = useCallback(
    (override: Partial<LectureFilters>) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                navigate({ query: e.currentTarget.value });
              }
            }}
            onBlur={(e) => {
        ...
        <div className="w-full sm:w-48">
          ...
          <select
            ...
            onChange={(e) =>
              navigate({ sort: e.target.value as LectureSortOption })
            }
            className={cn(
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {/* Scholar dropdown (too many for chips) */}
        {scholars.length > 0 && (
          <div className="w-full sm:w-56 shrink-0">
            <label htmlFor="scholar-select" className="sr-only">
              Filter by scholar
            </label>
            <select
              id="scholar-select"
