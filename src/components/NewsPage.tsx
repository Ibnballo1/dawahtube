...
import { Search, Filter, Calendar, User, ArrowRight, TrendingUp, Globe, Users, BookOpen } from "lucide-react"
...
export default function NewsPage() {
  ...
  const categories = [
    ...
    { id: "events", name: "Events", count: newsItems.filter((n) => n.category === "events").length, icon: Calendar },
    {
      id: "charity",
      name: "Charity",
    ...
    { id: "youth", name: "Youth", count: newsItems.filter((n) => n.category === "youth").length, icon: Users },
  ]
      ...
      <section className="bg-gradient-to-r from-primary to-primary-dark text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Islamic News</h1>
            <p className="text-xl text-blue-100 mb-8">
              Stay updated with the latest news and developments from the global Muslim community.
            </p>
              ...
              {featuredNews.map((news) => (
                <Card key={news.id} className="group hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                  <div className="aspect-video bg-gradient-to-br from-primary-light to-primary overflow-hidden relative">
                    ...
                    />
                    <Badge className="absolute top-4 left-4 bg-red-500 text-white">Breaking</Badge>
                  </div>
                  <CardHeader>
                    ...
                    <div className={`${index === 0 ? "md:w-1/2" : "md:w-2/3"}`}>
                      <CardHeader>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {news.featured && <Badge className="bg-red-100 text-red-800">Featured</Badge>}
                        </div>
                        <CardTitle
                          className={`${index === 0 ? "text-2xl" : "text-xl"} group-hover:text-primary transition-colors`}
                        >
                          {news.title}
                        </CardTitle>
                      </CardHeader>
